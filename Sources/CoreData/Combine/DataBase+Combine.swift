...
public extension DB {
	...
	func operation(queue: DispatchQueue = .main, operation: @escaping (_ context: NSManagedObjectContext, _ save: SaveBlock) throws -> Void) -> Future<Void, Error> {
		Future { promise in
	...
	/// - parameter queue: The queue on which your completion handler is dispatched.
	func saveToPersistentStore(_ moc: NSManagedObjectContext, queue: DispatchQueue = .main) -> Future<Void, Error> {
		Future { promise in
